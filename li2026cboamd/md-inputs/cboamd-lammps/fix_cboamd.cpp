  ...
  output_file(nullptr)
{
  ...
  int iarg = 3;
  while (iarg < narg) {
    if (strcmp(arg[iarg], "potential") == 0) {
      if (iarg + 1 >= narg) error->all(FLERR,"Illegal fix cboamd command");
      model_potential = utils::strdup(arg[iarg + 1]);
      iarg += 2;
  ...
  if (!model_potential) error->all(FLERR,"fix cboamd requires potential model");
  ...
  if (photons_enabled) {
    for (int i = 0; i < nphoton; i++) {
      fprintf(output_file, " qa_%d pa_%d ea_%d", i, i, i);
    }
  }
  fprintf(output_file, "\n");
}
...
void FixCBOAMD::post_force_respa(int n, int first, int last)
{
  if (n == 0) post_force(vflag);
}
...
int FixCBOAMD::unpack_restart(double *buf)
{
  // Unpack restart data
  int n = 0;
  for (int k = 0; k < 3; k++) {
    forces_deepmd[n * 3 + k] = buf[n++];
  }
  return n;
}
...
void FixCBOAMD::init_deepmd_models()
{
  try {
    ...
    deepmd_potential = new deepmd::DeepPot(model_potential);
    deepmd_dipole = new deepmd::DeepDipole(model_dipole);
    deepmd_polar = new deepmd::DeepPolar(model_polar);
...
void FixCBOAMD::compute_deepmd_energy_forces()
{
  // Convert coordinates to DeepMD format
  convert_coordinates_to_deepmd_format();
  
  try {
    // Compute energy and forces using DeepMD
    deepmd_potential->compute(energy_deepmd, forces_deepmd_vec, 
                             coords_deepmd, atom_types_deepmd, cell_deepmd);
    
    // Convert forces back to LAMMPS format
    convert_forces_from_deepmd_format();
    ...
    for (int i = 0; i < nlocal; i++) {
      for (int k = 0; k < 3; k++) {
        f[i][k] += forces_deepmd[i * 3 + k];
      }
    }
  } catch (const std::exception& e) {
    error->all(FLERR, "DeepMD potential computation failed: {}", e.what());
  }
}
...
void FixCBOAMD::compute_deepmd_dipole()
{
  try {
    ...
    dipole[0] = dipole_deepmd[0] * EV_PER_ANGSTROM_TO_HARTREE_PER_BOHR;
    dipole[1] = dipole_deepmd[1] * EV_PER_ANGSTROM_TO_HARTREE_PER_BOHR;
    dipole[2] = dipole_deepmd[2] * EV_PER_ANGSTROM_TO_HARTREE_PER_BOHR;
  } catch (const std::exception& e) {
...
void FixCBOAMD::compute_deepmd_polarizability()
{
  try {
    ...
    deepmd_polar->compute(polar_deepmd, polar_grad_deepmd, 
                         coords_deepmd, atom_types_deepmd, cell_deepmd);
...
void FixCBOAMD::compute_cboa_forces()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    for (int k = 0; k < 3; k++) {
      // Add CBOA forces to existing forces
      // This would be computed based on dipole gradients and polarizability gradients
      f[i][k] += 0.0;  // Dummy CBOA force
    }
  }
}
...
void FixCBOAMD::write_output()
{
  ...
  if (comm->me == 0) {
    ...
    if (photons_enabled) {
      for (int i = 0; i < nphoton; i++) {
        fprintf(output_file, " %.6e %.6e %.6e", qa[i], pa[i], ea[i]);
      }
    }
    fprintf(output_file, "\n");
    fflush(output_file);
...
void FixCBOAMD::convert_forces_from_deepmd_format()
{
  int nlocal = atom->nlocal;
  
  // Resize forces array
  forces_deepmd_vec.resize(nlocal * 3);
  
  // Convert forces from DeepMD format to LAMMPS format
  // DeepMD returns forces in eV/A, convert to LAMMPS units
  for (int i = 0; i < nlocal * 3; i++) {
    forces_deepmd[i] = -forces_deepmd_vec[i]; // DeepMD returns -dE/dr
  }
}
